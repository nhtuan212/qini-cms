...
export default function TargetItem({ target }: { target: TargetProps }) {
    //** Stores */
    const { isLoading } = useTargetShiftStore();
        ...
        >
            ...
            <div className="flex items-center gap-x-2">
                <Button
                    ...
                    color="default"
                    isIconOnly
                    onPress={() => setOpenTargetId(openTargetId === target.id ? null : target.id)}
                >
                    <ChevronRightIcon
                        className={twMerge(
            ...
            {openTargetId === target.id && <TargetShiftItem targetShifts={target.targetShift} />}
        </div>
    );
}
